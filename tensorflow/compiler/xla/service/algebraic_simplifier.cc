...
AlgebraicSimplifierVisitor::OptimizeDotOfReorderContractingDims(
  ...
  for (auto d : dnums.lhs_contracting_dimensions()) {
    lhs_contracting_dims.push_back(d);
  }
  std::vector<int64> rhs_contracting_dims;
  for (auto d : dnums.rhs_contracting_dimensions()) {
    rhs_contracting_dims.push_back(d);
  }
  ...
  lhs_contracting_dims.resize(reshape->operand(0)->shape().rank() -
  ...
  lhs_contracting_dims = ComposePermutations(lhs_contracting_dims, permutation);
  lhs = lhs->mutable_operand(0);
